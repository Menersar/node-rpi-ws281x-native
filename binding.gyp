    ...
    ['OS=="linux"', {
      'targets': [
...

        {
          ...
          'type': 'none',
          'sources': ['./node_modules/src/rpi_ws281x/version'],
          'actions': [{
            'action_name': 'make_version_h',
            'variables': {'version_file': './node_modules/src/rpi_ws281x/version'},
            'inputs': ['./tools/make-version-h.js', '<@(version_file)'],
            'outputs': ['./node_modules/src/rpi_ws281x/version.h'],
...

        {
          ...
          'sources': [
            "<!@(node -p \"require('fs').readdirSync('./node_modules/src/rpi_ws281x').filter(f => (f.match(/\.c$/) && !f.match('main.c'))).map(f => 'src/rpi_ws281x/' + f).join(' ')\")"
          ],
          'cflags!': ['-Wextra']
        }
      ]
